"""
Calculate and plot intra class correlation of FO between two sessions 
"""
import pandas as pd
import mne
import numpy as np
...
# Collect the ICC values from each parcel and FO values from the pre-central parcel
...
for sensor in range(0,len(lh_parc)):
    ...
    for low_high in ["low", "high"]:
        ...
        ICC_parcel = intraclass_corr(data=df, targets='subjects', raters='session', ratings='score').round(3)
