"""
Calculate and plot multi site stability of fractional occupancy 
"""
import pandas as pd
import mne
import numpy as np
...
FO_feature_df_site2 = pd.read_csv(fname.feature_csv(feature = 'FO',job_id=group_id+'_site2', task=task))
...
FO_values_all = {'low':{'site1':np.zeros((len(df_subjects), len(lh_parc))),
...
for low_high in ["low", "high"]:
    ...
    for i,row in df_subjects.iterrows():
        ...
        subject_df = FO_feature_df.loc[
            ...
            FO_feature_df['session']==1]
        
        sens_values = subject_df[columns].values[0]
        FO_values_all[low_high]['site1'][i,:] = np.array(np.nan_to_num(sens_values))
    
    for i,row in df_subjects_site2.iterrows():
        subject = row["subject"]
        subject_df = FO_feature_df_site2.loc[
            FO_feature_df_site2['subject']==subject].loc[
            FO_feature_df_site2['state_id']==low_high].loc[
            FO_feature_df_site2['session']==1]
        
        sens_values = subject_df[columns].values[0]
        FO_values_all[low_high]['site2'][i,:] = np.array(np.nan_to_num(sens_values))

for low_high in ["low", "high"]:
    p_corrected_list = []
    for sensor in range(0,len(lh_parc)):
        t, p_values = ttest_ind(a=FO_values_all[low_high]['site1'][:,sensor], b=FO_values_all[low_high]['site2'][:,sensor], axis=0)
...
for low_high in ["low", "high"]:
    ...
    # Plot also the average FO maps of the wo sites
    plot_limits = []
    for site in ['site1','site2']:
        FOs = np.mean(FO_values_all[low_high][site], axis=0)

        labels_to_stc = mne.labels_to_stc(labels=lh_parc, values=np.nan_to_num(np.array(FOs)), src=src)

        maxim = np.max(np.nan_to_num(np.array(FOs)))
        minim = np.nanmin(np.array(FOs))
    ...
    df = pd.DataFrame([(group, value[index_of_precentral]) for group, values in FO_values_all[low_high].items() for value in values],columns=['Group', 'Value'])
