"""
Calculate and plot multi site stability of SLT
"""
import pandas as pd
import mne
import numpy as np
...
SLT_feature_df_site2 = pd.read_csv(fname.feature_csv(feature = 'SLT',job_id=group_id+'_site2', task=task))
...
SLT_values_all = {'low':{'site1':np.zeros((len(df_subjects), len(lh_parc))),
...
for low_high in ["low", "high"]:
    for i,row in df_subjects.iterrows():
        ...
        subject_df = SLT_feature_df.loc[
            ...
            SLT_feature_df['session']==1].loc[
            SLT_feature_df['measure']=='mean']
        
        sens_values = subject_df[columns].values[0]*5
        SLT_values_all[low_high]['site1'][i,:] = np.array(np.nan_to_num(sens_values))
    
    for i,row in df_subjects_site2.iterrows():
        subject = row["subject"]
        subject_df = SLT_feature_df_site2.loc[
            SLT_feature_df_site2['subject']==subject].loc[
            SLT_feature_df_site2['state_id']==low_high].loc[
            SLT_feature_df_site2['session']==1].loc[
            SLT_feature_df_site2['measure']=='mean']
        
        sens_values = subject_df[columns].values[0]*5
        SLT_values_all[low_high]['site2'][i,:] = np.array(np.nan_to_num(sens_values))
    
for low_high in ["low", "high"]:
    p_corrected_list = []
    for sensor in range(0,len(lh_parc)):
        t, p_values = ttest_ind(a=SLT_values_all[low_high]['site1'][:,sensor], b=SLT_values_all[low_high]['site2'][:,sensor], axis=0)
...
for low_high in ["low", "high"]:
    ...
    # Plot also the average SLT maps of the wo sites
    plot_limits = []
    for site in ['site1','site2']:
        SLTs = np.mean(SLT_values_all[low_high][site], axis=0)

        labels_to_stc = mne.labels_to_stc(labels=lh_parc, values=np.nan_to_num(np.array(SLTs)), src=src)

        maxim = np.max(np.nan_to_num(np.array(SLTs)))
        minim = np.nanmin(np.array(SLTs))
    ...
    df = pd.DataFrame([(group, value[index_of_precentral]) for group, values in SLT_values_all[low_high].items() for value in values],columns=['Group', 'Value'])
