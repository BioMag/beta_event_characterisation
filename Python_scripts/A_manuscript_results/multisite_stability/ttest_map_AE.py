"""
Calculate and plot multi site stability of EA 
"""
import pandas as pd
import mne
import numpy as np
...
EA_feature_df = pd.read_csv(fname.feature_csv(feature = 'EA',job_id=group_id, task=task))
...
EA_feature_df_site2 = pd.read_csv(fname.feature_csv(feature = 'EA',job_id=group_id+'_site2', task=task))
...
EA_values_all = {'low':{'site1':np.zeros((len(df_subjects), len(lh_parc))),
...
for low_high in ["low", "high"]:
    ...
    for i,row in df_subjects.iterrows():
        subject = row["subject"]
        subject_df = EA_feature_df.loc[
            EA_feature_df['subject']==subject].loc[
            EA_feature_df['state_id']==low_high].loc[
            EA_feature_df['session']==1].loc[
            EA_feature_df['measure']=='mean'].loc[
            EA_feature_df['perc']==75]
        
        sens_values = subject_df[columns].values[0]
        EA_values_all[low_high]['site1'][i,:] = np.array(np.nan_to_num(sens_values))
    
    for i,row in df_subjects_site2.iterrows():
        subject = row["subject"]
        subject_df = EA_feature_df_site2.loc[
            EA_feature_df_site2['subject']==subject].loc[
            EA_feature_df_site2['state_id']==low_high].loc[
            EA_feature_df_site2['session']==1].loc[
            EA_feature_df_site2['measure']=='mean'].loc[
            EA_feature_df_site2['perc']==75]
        
        sens_values = subject_df[columns].values[0]
        EA_values_all[low_high]['site2'][i,:] = np.array(np.nan_to_num(sens_values))

for low_high in ["low", "high"]:
    p_corrected_list = []
    for sensor in range(0,len(lh_parc)):
        t, p_values = ttest_ind(a=EA_values_all[low_high]['site1'][:,sensor], b=EA_values_all[low_high]['site2'][:,sensor], axis=0)
...
for low_high in ["low", "high"]:
    ...
    # Plot also the average EA maps of the wo sites
    plot_limits = []
    for site in ['site1','site2']:
        EAs = np.mean(EA_values_all[low_high][site], axis=0)

        labels_to_stc = mne.labels_to_stc(labels=lh_parc, values=np.nan_to_num(np.array(EAs)), src=src)

        maxim = np.max(np.nan_to_num(np.array(EAs)))
        minim = np.nanmin(np.array(EAs))
    ...
    df = pd.DataFrame([(group, value[index_of_precentral]) for group, values in EA_values_all[low_high].items() for value in values],columns=['Group', 'Value'])
