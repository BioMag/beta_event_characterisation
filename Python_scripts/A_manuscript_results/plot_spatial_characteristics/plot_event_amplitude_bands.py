"""
Plot EA values on the low- and high-beta bands, across the whole brain
"""
import pandas as pd
import mne
import numpy as np
...
EA_features_df = pd.read_csv(fname.feature_csv(feature = 'EA',job_id=group_id, task=task))
session = sessions[0]
...
for low_high in ["whole_ts_high", "whole_ts_low"]:
    EA_values_all = np.zeros(len(parcels))

    for i,row in df_subjects.iterrows():
        subject = row["subject"]
        subject_df = EA_features_df.loc[
            EA_features_df['subject']==subject].loc[
            EA_features_df['state_id']=='low'].loc[
            EA_features_df['perc']==75].loc[
            EA_features_df['session']==int(session[1])].loc[
            EA_features_df['measure']==low_high]

        sens_values = subject_df[columns].values[0]
        EA_values_all += np.array(sens_values)

    EA_values_all = EA_values_all/len(df_subjects) # (i+1) 22/17

    # Put the parcel values to source estimate
    labels_to_stc = mne.labels_to_stc(labels=parcels, values=np.nan_to_num(np.array(EA_values_all)), src=src)

    # Create limits for th plot
    maxim = np.max(np.nan_to_num(np.array(EA_values_all)))
    minim = np.nanmin(np.array(EA_values_all))
